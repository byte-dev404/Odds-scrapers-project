...
async def enrich_base_pages_with_tabs(sport: str, base_pages: list[dict], browser: Browser, workers_count: int = 3) -> list[dict]:
    queue = asyncio.Queue()
    ...
    async def worker(worker_id: int):
        while True:
            ...
            try:
                ...
                context = await browser.new_context(extra_http_headers=headers_for_api)
                await context.add_cookies(convert_cookies(cookies_for_api))
        ...
        for sport_name, relative_path in sports.items():
            ...
            while True:
                ...
                try: 
                    ...
                    base_pages = await get_base_pages(sport_name, links, 4)
                    await asyncio.sleep(5.00)
                    enriched_pages = await enrich_base_pages_with_tabs(sport_name, base_pages, browser, max_match_workers)                
